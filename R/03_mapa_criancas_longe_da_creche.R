...
mapa_criancas_sem_escola <- function(muni, size = 16) {
  
  # centroide da cidade
  centroid_sf <- longe_sf %>%
    filter(sigla_muni == muni, pop_criancas > 0) %>%
    summarise() %>%
    st_centroid()
...
mapa_criancas_sem_escola("for")

m_rec + m_poa + m_slz + m_cam + plot_layout(ncol = 2, guides = "collect")
